...
class CommandFunctions:
    ...
    def admins_cmd() -> list[str]:
        return [
            '~name', '~imp', '~box', '~adda',
            '~deladmin', '~kill', '~pause', '~infoh',
            '~infod', '~info', '~infc', '~end',
            '~kick', '~chatL', '~fz', '~pc',
            '~maxp', '~smsg', '~sleep',
            '/mute', '/unmute', '~gm', '~sm', '~sp',
            '~et', '~punch', '~mbox', '~drop', '~gift',
            '/curse', '~superj',
            ]
...
def _install() -> None:
    ...
    if Uts.key not in read:
        ...
        ix = read_l.index('def filter_chat_message(msg: str, client_id: int) -> Optional[str]:') 
